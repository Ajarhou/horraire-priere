...
export default function Prierres() {
  ...
  useEffect(() => {
    axios
      .get(
        `https://api.aladhan.com/v1/timingsByCity/29-01-2025?country=FR&city=${city}`
      )
      .then((response) => {
  ...
  const setUpCountDownTimer = () => {
    ...
    if (DifferenceTimes < 0) {
      ...
      const fajrTomidNightDiff = nextPrayerTimesMoment.diff(
    ...
    setRemaianing(
      `${durationTimes.hours()} : ${durationTimes.minutes()} : ${durationTimes.seconds()}`
    );
  ...
  return (
    <>
      <Grid container>
        ...
        <Grid size={6}>
          <div>
            <h2>Reste jusqu'au prochaine priere {nextPrayer}</h2>
